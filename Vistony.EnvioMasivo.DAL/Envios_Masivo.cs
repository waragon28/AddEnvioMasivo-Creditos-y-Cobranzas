...
namespace Vistony.Masivo.DAL
{
    public class Envios_Masivo
    {
        ...
        public Campaigns ObtenerCabeceraCampania_Cre(SAPbouiCOM.Grid dt, string mensaje,
    string CampaignName, string Owner, string AssignName)
        {
            List<Campaigns> ListCampaignsDocument = new List<Campaigns>();
            Campaigns CampaignsDocument = new Campaigns();
            ...
            CampaignsDocument.Remarks = mensaje;
            CampaignsDocument.TargetGroupType = "tgtCustomer";
            CampaignsDocument.CampaignBusinessPartners = ObtenerDetalleCompania_Cre(dt, AssignName);
            return CampaignsDocument;
        }
        public List<CampaignBusinessPartners> ObtenerDetalleCompania_Cre(SAPbouiCOM.Grid dt,
            string AssignName)
        {
            List<CampaignBusinessPartners> ListtransferDocumentDetalls = new List<CampaignBusinessPartners>();
            for (int oRows = 0; oRows < dt.Rows.Count; oRows++)
            {
                CampaignBusinessPartners transferDocumentDetalls = new CampaignBusinessPartners();
                string GetTelef = dt.DataTable.GetValue("Telefono", oRows).ToString();
                if (GetTelef.Length == 9 && dt.DataTable.GetString("Marcar", oRows) == "Y")
                {
                    transferDocumentDetalls.BPCode = dt.DataTable.GetValue("CodCliente", oRows).ToString();
                    transferDocumentDetalls.BPGroupName = "Clte.Local Tercero";
                    transferDocumentDetalls.BPStatus = "A";
                    transferDocumentDetalls.AssignName = AssignName;
                    transferDocumentDetalls.CreateActivity = "tNO";
            ...
            SAPbouiCOM.StaticText StaticText10, SAPbouiCOM.EditText EditText4)
        {
            ...
            ObtenerCabecera2 = tr.ObtenerCabeceraCampania_Cre(dt, EditText0.Value, CampaignName,
        ...
        public void FormatoGrilla2(SAPbouiCOM.Grid Grid1)
        {
            // Grid1.AssignLineNro();
            Grid1.Columns.Item(0).Type = SAPbouiCOM.BoGridColumnType.gct_CheckBox;
            Grid1.Columns.Item(2).LinkedObjectType(Grid1, "CodCliente", "2");
            Grid1.AutoResizeColumns();
            Grid1.ReadOnlyColumns();
            Grid1.Columns.Item(0).Editable = true;
        }
        ...
        public void Get_Cliente_ClienteesCredito(SAPbouiCOM.DataTable oDatatable, SAPbouiCOM.Grid Grid1, SAPbouiCOM.Form oForm, string FechaInicio,
                                                 string FechaFin,string UnidadNegocio)
        {
            Sb1Messages.ShowWarning("Cargando Clientes");
            try
            {
                ...
                if (UnidadNegocio=="")
                {
                    strHANA = string.Format("CALL P_VIS_ADDON_ENVIO_SMS_CLIENTES_MOROSOS('{0}','{1}','{2}')", FechaInicio, FechaFin, UnidadNegocio);
                ...
                oDatatable = oForm.DataSources.DataTables.Item("DT_1");
                oForm.Freeze(true);
                oDatatable.ExecuteQuery(strHANA);
                FormatoGrilla2(Grid1);
                
                for (int oRows = 0; oRows < Grid1.Rows.Count -1; oRows++)
                {

                    Data transferDocumentDetalls2 = new Data();
                    if (Convert.ToString(Grid1.DataTable.GetValue("Marcar", oRows)) == "N")
                    {
                        Grid1.SetRowBackColor(oRows, 198164156);
                    }
                    else
        ...
        public void EnviarSMSCREDITOS(SAPbouiCOM.EditText RespuestMensaje, SAPbouiCOM.Grid Grid1,SAPbouiCOM.EditText Mensaje)
        {
            Cabecera_Mensaje ObtenerCabecera2 = new Cabecera_Mensaje();
            ...
            try
            {
                ...
                for (int oRows = 0; oRows < Grid1.Rows.Count ; oRows++)
                {
                    ...
                    Data transferDocumentDetalls2 = new Data();
                    if (Convert.ToString(Grid1.DataTable.GetValue("Marcar", oRows)) == "Y")
                    {
                        string s = Mensaje.Value.ToString();
                       ...
                       "#Vistony #TecnologiaQueGeneraConfianza." + "\n\n" +
                       "https://bit.ly/3L7TBNH";*/
                        transferDocumentDetalls2.NumeroTelf = "51" + "942554237";// Convert.ToString(Grid1.DataTable.GetValue("Telefono", oRows));
                        ListtransferDocumentDetalls2.Add(transferDocumentDetalls2);
                    }
